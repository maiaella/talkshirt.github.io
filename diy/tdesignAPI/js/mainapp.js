	...
	$y_pos="front";
	 html2canvas($('#preview_front'), {
            onrendered: function(canvas) {
				...
				// $('#sendfile').val(canvas.toDataURL("image/png"));

				$('#imagessend').src = canvas.toDataURL("image/png");


    $('#imagessend').attr('src', canvas.toDataURL("image/png"));


$('#imageFileForm').on('submit',function(){
	var preview_front = document.getElementById('preview_front');
	var dataURI = canvas.toDataURL();
	$('#preview_front_image').val(dataURI);
    ...
    html2canvas($('#preview_back'), {
            onrendered: function(canvas) {
				...
				$('#imageFileForm').on('submit',function(){
					var preview_back = document.getElementById('preview_back');
					var dataURI = canvas.toDataURL();
					$('#preview_back_image').val(dataURI);
